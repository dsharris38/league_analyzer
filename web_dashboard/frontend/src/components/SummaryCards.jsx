            ...
            {cards.map((card) => (
                <div key={card.label} className="glass-panel p-5 rounded-2xl glass-card-hover group">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-slate-400 text-sm font-medium group-hover:text-cyan-200 transition-colors">{card.label}</span>
                        <div className={`p-2 rounded-lg ${card.bg} ${card.color} group-hover:scale-110 transition-transform duration-300 ring-1 ring-white/5`}>
                            <card.icon size={18} />
                        </div>
                    </div>
